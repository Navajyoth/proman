...
def run(*args):
    api = Bitbucket(args[0], args[1])
    result = api.get_commits('rawdatatech', 'proman')
    for item in result['values']:
        commit = Commit()
        commit.url = item['links']['html']['href']
        commit.username = item['author']['raw'].split()[0]
        commit.date = item['date']
        commit.comment = item['message']
        commit.save()
