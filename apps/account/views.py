...
from .models import EmailAddress, EmailConfirm, PasswordReset, User, UserTechnology
from .serializers import UserSerializer, UserTechnologySerializer
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    queryset = User.objects.get_visible()

    @detail_route(methods=['GET'])
    def technologies(self, request, pk):
        user = self.get_object()
        technologies = user.technologies.all()
        slz = UserTechnologySerializer(technologies, many=True)
...
class UserTechnologyViewSet(viewsets.ModelViewSet):
    serializer_class = UserTechnologySerializer
    permission_classes = (IsAuthenticated,)
    queryset = UserTechnology.objects.all()


class RegisterView(AbstractFormView):
    template_name = 'form_edit.html'
    form_class = RegistrationForm
    success_message = 'Thanks for resgistering with us. Complete the regisration\
     process by responding to the verfication email.'
