...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    is_manager = models.BooleanField(
        _('manager'), default=False,
        help_text=_('Designates whether user is manager.'))
    is_active = models.BooleanField(
        _('active'), default=True,
        help_text=_('Designates whether user should be treated as active.'))
    date_joined = models.DateTimeField(_('date joined'), default=timezone.now)
    is_visible = models.BooleanField(default=True)
    slack_id = models.CharField(max_length=64, null=True, blank=True)
...
class UserTechnology(models.Model):
    ...
    technology = models.ForeignKey(Technology, related_name='users')


@receiver(post_save, sender=settings.AUTH_USER_MODEL)
def create_auth_user_model(sender, instance=None, created=False, **kwargs):
    if created:
